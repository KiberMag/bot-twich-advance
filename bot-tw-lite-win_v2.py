...
import logging
import random
import time  # Импортируем библиотеки для определения времени
import threading
...
class TwitchBotGUI:
    def __init__(self, root):
        ...
        # self.bot_owner_var = tk.StringVar(value="kibermag")
        # self.author_name_var = tk.StringVar(value="kibermag")
    ...
    def setup_control_tab(self, parent):
        ...
        self.stop_btn = ttk.Button(control_frame, text="Авторизация бота", command=self.twich_avtorization_bot, state=tk.DISABLED)
        self.stop_btn.pack(side=tk.LEFT, padx=5)
